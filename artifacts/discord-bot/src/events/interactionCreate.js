  ...
  async execute(interaction, client) {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket_type') {
      ...
      if (db.getTicketCounter) {
        ...
        // Fallback safety if database tracking doesn't support a dedicated counter
      ...
      // Base permission parameters. Creator always has explicit channel access.
      ...
      // Exclusively grant ticket administration permissions to explicit Staff Roles
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_claim') {
      ...
      // Re-render Control panel with updated claim label (Disable claims)
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_close_request') {
      ...
      // Case 1: Creator requests close -> Prompt Confirmation Flow
      if (isCreator && !isStaffUser) {
      ...
      // Case 2: Staff requests close -> Skip verification confirmation and trigger immediate count down
      await interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(config.colors.error)
            .setTitle('🔒 Ticket Closing')
            .setDescription(`This ticket has been scheduled to close by staff member **${interaction.user.tag}**.\n\nChannel will be deleted in **5 seconds**.`)
            .setTimestamp()
        ]
      });
      ...
      const logEmbed = new EmbedBuilder()
        ...
        .setDescription(`Ticket channel **${interaction.channel.name}** is closing.`)
        .addFields(
    ...
    // ── Confirm Close Button (Creator) ────────────────────────────────────────
    if (interaction.isButton() && interaction.customId === 'ticket_confirm_close') {
      const ticket = db.getTicket ? db.getTicket(interaction.channel.id) : null;
      if (ticket && interaction.user.id !== ticket.userId) {
        return interaction.reply({ embeds: [new EmbedBuilder().setColor(config.colors.error).setDescription('❌ Only the ticket creator can confirm this.')], ephemeral: true });
      }

      await interaction.update({ content: '🔒 **Closing ticket...** Channel will delete in 5 seconds.', components: [] });

      // Send public status log notice inside channel
      await interaction.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(config.colors.error)
            .setTitle('🔒 Ticket Closing')
            .setDescription(`This ticket has been marked closed by owner **${interaction.user.tag}**.\n\nChannel will delete in **5 seconds**.`)
            .setTimestamp()
        ]
      });
      ...
      const logEmbed = new EmbedBuilder()
        .setColor(config.colors.error)
        .setTitle('🔒 Ticket Closed')
        .setDescription(`Ticket channel **${interaction.channel.name}** closed by owner.`)
        .addFields({ name: 'Owner', value: `${interaction.user.tag}`, inline: true })
        .setTimestamp();
      await sendTicketLog(guild, logEmbed);

      if (db.closeTicket) db.closeTicket(interaction.channel.id);
      setTimeout(() => interaction.channel.delete('Ticket closed by user.').catch(() => {}), 5000);
      return;
    }

    // ── Cancel Close Button (Creator) ─────────────────────────────────────────
    if (interaction.isButton() && interaction.customId === 'ticket_cancel_close') {
      const ticket = db.getTicket ? db.getTicket(interaction.channel.id) : null;
      if (ticket && interaction.user.id !== ticket.userId) {
        return interaction.reply({ embeds: [new EmbedBuilder().setColor(config.colors.error).setDescription('❌ Only the ticket creator can cancel this.')], ephemeral: true });
      }
